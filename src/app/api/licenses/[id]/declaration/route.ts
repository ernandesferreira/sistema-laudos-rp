...
import { asHttpError, fail } from "@/lib/http";
...
export async function GET(request: Request, context: Context) {
  try {
    let authUser = await requireApiPermission(request, "licenses.view");
    ...
    const declarationData = await requestService.getDeclarationData(releasedLicense.serviceRequestId);
